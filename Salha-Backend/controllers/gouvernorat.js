...
exports.gouvparid= async(req, res, next) => {
  try {
  const [unegouv] = await gouvernorat.getgouv(req.params.id_G);
  res.status(200).json(unegouv);
  ...
  exports.deletegouv = async (req, res, next) => {
    try {
      const deleteResponse = await gouvernorat.delete(req.params.id_G);
